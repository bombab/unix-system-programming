...
#define StoC1_QKEY  (ket_t)60142 // server->cleint1 작업 큐의 key 값
#define StoC2_QKEY  (ket_t)60143 // server->cleint2 작업 큐의 key  값
...
typedef struct { // 메시지 큐로 send 할 메시지중 실질 메시지 내용
    int result;
    char calc_method;
} cal_result_msg;

typedef struct {  // 메시지 큐로 send 할 메시지 타입
...
int Client2Server(key_t type, int cal_result, char cal_type) {   // client에서 server로 메시지를 전송한다.
    int s_qid;
    c2s_msg c2s;
    key_t type = type;
    if(s_qid = init_queue(type) == -1) return -1; // 메시지큐 새성
    ...
    else return 0; // 정상적으로 전송될 시 0을 반환


}

int Server2Client() { // server에서 client로 메시지 전송
    return 0;
}
